...
// Implements methods for printing electron-specific data and energy distribution
...
Electron::Electron(double E, double px, double py, double pz, bool antiparticle) 
  : Lepton("Electron", E, px, py, pz, antiparticle, -1), energy_deposited(4, 0.0), 
...
void Electron::print_energy_depositied() const 
{
  ...
  <<std::scientific<<std::setprecision(2)<<std::right
  <<std::setw(6)<<get_em_1()<<", "<<std::setw(6)<<get_em_2()<<", "
  <<std::setw(6)<<get_had_1()<<", "<<std::setw(6)<<get_had_2()
  <<"]"<<" GeV"<<std::fixed<<std::endl;
}
